...
sequenceDiagram
    ...
    loop Continuous Pre-Block Production
        ...
        alt Has transactions
            ...
            Block->>Subscribers: Broadcast via gossip
...
The CHOAM group (context) uses **Bounded Epidemic Gossip (BEG)** from the _membership_ module to distribute blocks to all live members using a 2/3+1 variation of the Fireflies ring calculation. This protocol's message buffer is bounded and garbage collected. As it is a garbage collected, bounded buffer gossip, the messages will ultimately age out and be discarded.
