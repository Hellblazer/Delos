...
public class GossipPropagationTest {
    ...
    public void gossipPropagationWithSequentialJoins() throws Exception {
        ...
        System.out.println("Seeds stabilized. Starting sequential joins...");

        // Join nodes ONE AT A TIME with stabilization between each
        for (int i = SEED_COUNT; i < CARDINALITY; i++) {
            final int idx = i;
            ...
            assertTrue(Utils.waitForCondition(30_000, 1_000, () -> {
                return views.get(idx).getContext().activeCount() >= expectedSize - 1; // Allow some lag
            }), "Node " + i + " did not receive full membership");

            System.out.printf("  Node %d joined: size=%d active=%d%n",
                             i, views.get(i).getContext().size(),
                             views.get(i).getContext().activeCount());
        }
        ...
        assertTrue(converged, "Sequential joins did not converge to " + CARDINALITY + " members");
    }
